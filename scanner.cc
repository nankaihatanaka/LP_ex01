...
/*******************************************************************
  この次の行に，課題作成に参加したメンバの学籍番号を記入すること．
s1210187 Yuki Yamashita
s1210223 Minami Hatanaka
s1210240 Satoshi Takara
*******************************************************************/
...
void initializeScanner(char *filename)
{
  ...
  //原始プログラムファイルオープンし、失敗したらエラー表示
  if((fp=fopen(filename,"r"))==NULL){
    printf("EFileNotFound\n");
    exit(EXIT_FAILURE);
  }
  //オープンしたらファイルから1文字読み、currentCharに代入
  fgets(s,1,fp);
  currentChar = s;
  srcFilePointer = fp;
  lineNo=1;
